...
type BucketInfoDetailInner struct {
	// Name of the bucket.
	Name string
	...
	// storageclass: standard or other..
	StorageClass string
...
type BucketInfoDetail struct {
	XMLName xml.Name              `xml:"BucketInfo"`
	Bucket  BucketInfoDetailInner `xml:"Bucket"`
}

func (c *Client) GetBucketInfo(ctx context.Context, bucketName string) (*BucketInfoDetail, error) {
	...
	bucketInfoDetail := &BucketInfoDetail{}
	err = xmlDecoder(resp.Body, bucketInfoDetail)
	if err != nil {
		return nil, err
	}
	return bucketInfoDetail, nil
}
	...
	go func(objectStatCh chan<- ObjectInfo) {
		...
		for {
			...
			result, err := c.listObjectsV2Query(ctx, bucketName, opts.Prefix, continuationToken,
				fetchOwner, opts.WithMetadata, delimiter, opts.StartAfter, opts.MaxKeys, opts.FetchDelete, opts.headers)
			if err != nil {
				objectStatCh <- ObjectInfo{
					Err: err,
				}
...
func (c *Client) listObjectsV2Query(ctx context.Context, bucketName, objectPrefix, continuationToken string, fetchOwner, metadata bool, delimiter string, startAfter string, maxkeys int, fetchDelete bool, headers http.Header) (ListBucketV2Result, error) {
	// Validate bucket name.
	if err := s3utils.CheckValidBucketName(bucketName); err != nil {
	...
	if fetchDelete {
		urlValues.Set("fetch-delete", "true")
	}
...
func (c *Client) listObjectVersions(ctx context.Context, bucketName string, opts ListObjectsOptions) <-chan ObjectInfo {
	...
	go func(resultCh chan<- ObjectInfo) {
		...
		for {
			// Get list of objects a maximum of 1000 per request.
			result, err := c.listObjectVersionsQuery(ctx, bucketName, opts.Prefix, keyMarker, versionIDMarker, delimiter, opts.MaxKeys, false, opts.headers)
			if err != nil {
				resultCh <- ObjectInfo{
					Err: err,
				}
...
// ?prefix - Limits the response to keys that begin with the specified prefix.
// ?max-keys - Sets the maximum number of keys returned in the response body.
func (c *Client) listObjectVersionsQuery(ctx context.Context, bucketName, prefix, keyMarker, versionIDMarker, delimiter string, maxkeys int, fetchDelete bool, headers http.Header) (ListVersionsResult, error) {
	// Validate bucket name.
	if err := s3utils.CheckValidBucketName(bucketName); err != nil {
...
type ListObjectsOptions struct {
	// Inculde Objects delete in the listing
	FetchDelete bool
